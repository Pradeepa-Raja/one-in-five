<?php
/**
 * The Template for displaying all single posts.
 *
 * @package VW Charity NGO
 */
 
 get_header(); ?>

<div class="container">
  <div class="middle-align">
    <?php
        $theme_lay = get_theme_mod( 'vw_charity_ngo_theme_options','Right Sidebar');
        if($theme_lay == 'Left Sidebar'){ ?>
        <div class="row">
          <div class="col-md-4 col-sm-4" id="sidebar"><?php dynamic_sidebar('sidebar-1');?></div>
          <div id="our-services" class="services col-md-8 col-sm-8">
                    
            <?php if ( have_posts() ) :
              /* Start the Loop */
                
                while ( have_posts() ) : the_post();

                  get_template_part( 'template-parts/single-post-layout' ); 
                
                endwhile;

                else :

                  get_template_part( 'no-results', 'archive' ); 

                endif; 
            ?>
			  <h1><?php the_title(); ?></h1>
			   <div class="thumb-image"> <?php if ( has_post_thumbnail() ) {
	the_post_thumbnail();
}  ?></div>
			  <div class="custom-location-field"><strong>Place:</strong> <?php the_field('location'); ?></div>
			  <div class="custom-location-field"><strong>Date:</strong> <?php the_field('date'); ?></div>
			  <div class="custom-location-field"><?php the_field('price'); ?></div>
			<div class="clear"></div>
			  <?php the_content(); ?>
            <div class="navigation">
              <?php
                  // Previous/next page navigation.
                  the_posts_pagination( array(
                      'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                      'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                      'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                  ) );
              ?>
                <div class="clearfix"></div>
            </div>
          </div>
        </div>
        <div class="clearfix"></div>
    <?php }else if($theme_lay == 'Right Sidebar'){ ?>
        <div class="row">
          <div id="our-services" class="services col-md-8 col-sm-8">
                      
            <?php if ( have_posts() ) :
              /* Start the Loop */
                
                while ( have_posts() ) : the_post();

                  get_template_part( 'template-parts/single-post-layout' ); 
                
                endwhile;

                else :

                  get_template_part( 'no-results', 'archive' ); 

                endif; 
            ?>
			  <h1><?php the_title(); ?></h1>
			  <div class="thumb-image"> <?php if ( has_post_thumbnail() ) {
	the_post_thumbnail();
}  ?></div>
			<div class="custom-location-field"><strong>Place:</strong> <?php the_field('location'); ?></div>
			  <div class="custom-location-field"><strong>Date:</strong> <?php the_field('date'); ?></div>
			  <div class="custom-location-field"><?php the_field('price'); ?></div>
			  <div class="clear"></div>
			   <?php the_content(); ?>
			  
            <div class="navigation">
              <?php
                  // Previous/next page navigation.
                  the_posts_pagination( array(
                      'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                      'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                      'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                  ) );
              ?>
                <div class="clearfix"></div>
            </div>
          </div>
          <div class="col-md-4 col-sm-4" id="sidebar"><?php dynamic_sidebar('sidebar-1');?></div>
        </div>
    <?php }else if($theme_lay == 'Right Sidebar'){ ?>
        <div id="our-services" class="services">
                    
          <?php if ( have_posts() ) :
            /* Start the Loop */
              
              while ( have_posts() ) : the_post();

                get_template_part( 'template-parts/single-post-layout' ); 
              
              endwhile;

              else :

                get_template_part( 'no-results', 'archive' ); 

              endif; 
          ?>
          <div class="navigation">
            <?php
                // Previous/next page navigation.
                the_posts_pagination( array(
                    'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                    'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                    'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                ) );
            ?>
              <div class="clearfix"></div>
          </div>
        </div>
    <?php }else if($theme_lay == 'Three Columns'){ ?>
        <div class="row">
          <div class="col-md-3 col-sm-3" id="sidebar"><?php dynamic_sidebar('sidebar-1');?></div>
          <div id="our-services" class="services col-md-6 col-sm-6">
			  <h1><?php the_title(); ?></h1>
			   <div class="thumb-image"> <?php if ( has_post_thumbnail() ) {
	the_post_thumbnail();
}  ?></div>
			 <div class="custom-location-field"><strong>Place:</strong> <?php the_field('location'); ?></div>
			  <div class="custom-location-field"><strong>Date:</strong> <?php the_field('date'); ?></div>
			  <div class="custom-location-field"><?php the_field('price'); ?></div>
			   <div class="clear"></div>
           <?php the_content(); ?>
			  
            <?php if ( have_posts() ) :
              /* Start the Loop */
                
                while ( have_posts() ) : the_post();

                  get_template_part( 'template-parts/single-post-layout' ); 
                
                endwhile;

                else :

                  get_template_part( 'no-results', 'archive' ); 

                endif; 
            ?>
            <div class="navigation">
              <?php
                  // Previous/next page navigation.
                  the_posts_pagination( array(
                      'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                      'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                      'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                  ) );
              ?>
                <div class="clearfix"></div>
            </div>
          </div>
          <div class="col-md-3 col-sm-3" id="sidebar"><?php dynamic_sidebar('sidebar-2');?></div>
        </div>
    <?php }else if($theme_lay == 'Four Columns'){ ?>
        <div class="row">
          <div class="col-md-3 col-sm-3" id="sidebar"><?php dynamic_sidebar('sidebar-1');?></div>
          <div id="our-services" class="services col-md-3 col-sm-3">
			  <h1><?php the_title(); ?></h1>
			   <div class="thumb-image"> <?php if ( has_post_thumbnail() ) {
	the_post_thumbnail();
}  ?></div>
			 <div class="custom-location-field"><strong>Place:</strong> <?php the_field('location'); ?></div>
			  <div class="custom-location-field"><strong>Date:</strong> <?php the_field('date'); ?></div>
			  <div class="custom-location-field"><?php the_field('price'); ?></div>
			   <div class="clear"></div>
                <?php the_content(); ?>   
			  
            <?php if ( have_posts() ) :
              /* Start the Loop */
                
                while ( have_posts() ) : the_post();

                  get_template_part( 'template-parts/single-post-layout' ); 
                
                endwhile;

                else :

                  get_template_part( 'no-results', 'archive' ); 

                endif; 
            ?>
            <div class="navigation">
              <?php
                  // Previous/next page navigation.
                  the_posts_pagination( array(
                      'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                      'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                      'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                  ) );
              ?>
                <div class="clearfix"></div>
            </div>
          </div>
          <div class="col-md-3 col-sm-3" id="sidebar"><?php dynamic_sidebar('sidebar-2');?></div>
          <div class="col-md-3 col-sm-3" id="sidebar"><?php dynamic_sidebar('sidebar-3');?></div>
        </div>
    <?php }else if($theme_lay == 'Grid Layout'){ ?>
        <div class="row">
          <div id="our-services" class="services col-md-9">
            <div class="row">
              <?php if ( have_posts() ) :
                /* Start the Loop */
                  
                  while ( have_posts() ) : the_post();

                    get_template_part( 'template-parts/single-post-layout' ); 
                  
                  endwhile;

                  else :

                    get_template_part( 'no-results', 'archive' ); 

                  endif; 
              ?>
				<h1><?php the_title(); ?></h1>
				 <div class="thumb-image"> <?php if ( has_post_thumbnail() ) {
	the_post_thumbnail();
}  ?></div>
				<div class="custom-location-field"><strong>Place:</strong> <?php the_field('location'); ?></div>
			  <div class="custom-location-field"><strong>Date:</strong> <?php the_field('date'); ?></div>
			  <div class="custom-location-field"><?php the_field('price'); ?></div>
				 <div class="clear"></div>
				 <?php the_content(); ?>
				
            </div>
            <div class="navigation">
              <?php
                  // Previous/next page navigation.
                  the_posts_pagination( array(
                      'prev_text'          => __( 'Previous page', 'vw-charity-ngo' ),
                      'next_text'          => __( 'Next page', 'vw-charity-ngo' ),
                      'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'vw-charity-ngo' ) . ' </span>',
                  ) );
              ?>
                <div class="clearfix"></div>
            </div>
          </div>
          <div class="col-md-3" id="sidebar"><?php dynamic_sidebar('sidebar-1');?></div>
        </div>
    <?php } ?>
    <div class="clearfix"></div>
  </div>
</div>

<?php get_footer(); ?>